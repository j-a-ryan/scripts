...
with open(full_file_path, mode="r") as raw_file:
    ...
    raw_transmitter_names_str = (
        raw_file.readlines()[1].strip("\n").strip().replace('"', "")
    )  # A line has been read; the new line 1 should have the transmitter names.

    raw_transmitter_names_str = raw_transmitter_names_str.removeprefix(",")
...
smoothed_df = util.smooth(
    truncated_file_df=truncated_file_df,  # Create a dataframe that contains the smoothed data.
...
for i in range(len(temp_field_names)):  # assumes lw and temp name arrays same length
    nonzero_risk_period_for_this_lw_temp_pair = util.analyze(
        analysis_df=analysis_df, leaf_wetness_field_name=leaf_wetness_field_names[i], temp_field_name=temp_field_names[i]
    )
...
for ranking in rankings:
    highest_rank_wet = max(ranking[0], highest_rank_wet)
    highest_rank_with_dry_time = max(ranking[1], highest_rank_with_dry_time)
    highest_rank_with_half_dry_time = max(ranking[1], highest_rank_with_half_dry_time)
...
for i in range(len(nonzero_risk_periods)):
    ...
    nonzero_risk_period = nonzero_risk_periods[i]
    if len(nonzero_risk_period) > 0:
        for value in nonzero_risk_period.values():
            total_ru = value[0]
            dry_ru = value[1]
            end_time = value[2]
        ...
        net_all_sensor_pairs_infection_period_ru = max(infection_period_ru, net_all_sensor_pairs_infection_period_ru)
        net_all_sensor_pairs_infection_period_ru_wet = max(infection_period_ru_wet, net_all_sensor_pairs_infection_period_ru_wet)
        net_all_sensor_pairs_infection_period_ru_avg = max(infection_period_ru_avg, net_all_sensor_pairs_infection_period_ru_avg)
