...
def analyze(analysis_df: pd.DataFrame, leaf_wetness_field_name, temp_field_name) -> dict:
    ...
    for i in range(num_rows):
        ...
        if lw > 0 or (risk_units_running_total > 0 and num_dry_hours < num_dry_hours_to_stop_cycle):
            if (
                temp in lookup.risk_units_lookup
            ):  # If it's not very cold or very hot (We are not considering heat as killing the fungus or stopping the infection)
                ...
                if (
                    i == num_rows - 1
                ):  # If at end of file, finish the risk period. It is continuing, but there is nothing we can do about that. Make a new, extended file that has the enire risk period in it.
                    nonzero_risk_periods_for_this_sensor_pair[start_time] = [
                        round(risk_units_running_total),
                        round(num_final_dry_risk_units),
                        datetime,
                    ]  # start time maps to array of ru, final dry ru, end time
        else:
...
def smooth(truncated_file_df: pd.DataFrame, temp_field_names, leaf_wetness_field_names):
    ...
    for index, row in truncated_file_df.iterrows():
        ...
        if (
            index % 4 == 0 or index == len(truncated_file_df) + 1
        ):  # If at beginning/end of group of 4, including end of file (may be group of fewer than 4 which we'll count as a group)
            if index > 0:  # If it's not the first row
                ...
                if smoothed_df.empty:
                    smoothed_df = new_row_df
                else:
                    smoothed_df = pd.concat([smoothed_df, new_row_df], ignore_index=True)
